...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchExaminee = async () => {
      try {
        const res = await axios.get(`https://examprep-9ld9.onrender.com/api/examinee/${examineeId}`);
        console.log(res.data.data);
        
        if (res.data) {
          setFormData(res.data.data);
          if (res.data.profileImage) {
            setProfilePic(`https://examprep-9ld9.onrender.com/uploads/${res.data.profileImage}`);
          }
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await axios.put(
        `https://examprep-9ld9.onrender.com/api/examinee/${examineeId}`,
        data,
