...
const Session = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editform) {
        const res = await axios.put(`https://examprep-9ld9.onrender.com/api/session/${id.id}`, form);
        if (res) {
        ...
        const res = await axios.post('https://examprep-9ld9.onrender.com/api/session', form)
        if (res) {
  ...
  const handlefetch = async () => {
    const res = await axios.get('https://examprep-9ld9.onrender.com/api/session')
  ...
  const handleDelete = async (id) => {
    // console.log(id)
    const res = await axios.delete(`https://examprep-9ld9.onrender.com/api/session/${id}`);
    if (res) {
